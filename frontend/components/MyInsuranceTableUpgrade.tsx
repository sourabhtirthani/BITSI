...
const MyInsuranceTableUpgrade = ({address} : {address : string}) => {
    ...
    useEffect(()=>{
      const getDataOfNftOnLoad = async()=>{
        try{
          if(address){
            const responseFromServer = await  fetch(`/api/userzone/insurance/upgrade/${address}`, { method: "GET", next: { revalidate: 0 }})
                  ...
                  return (
                    <React.Fragment key={index}>
                      <tr className='bg-success-512 text-center  secondary-shadow11 w-full text-white font-montserrat text-[12px] max-sm:text-[8px] font-semibold'>
                        ...
                        <td className='p-2 max-sm:p-1'><DialogUserZoneProtection action='upgrade' setRefresh={setRefresh} assetId={item.id} assetName={item.nft_name} /></td>
                        {/* <td>
