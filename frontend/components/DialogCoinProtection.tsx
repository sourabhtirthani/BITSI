...
const DialogCoinProtection = ({buttonText , action , handleMethodCall , coinInsuranceId , setRefresh, dialogTitle , dialogDescription , loaderActionButton } : {buttonText: string , action : string ,handleMethodCall : (id : number , setRefresh : React.Dispatch<React.SetStateAction<boolean>>)=>void , coinInsuranceId : number , setRefresh : React.Dispatch<React.SetStateAction<boolean>> , dialogTitle : string , dialogDescription : string , loaderActionButton : boolean  }) => {
            ...
            <DialogDescription>{dialogDescription}</DialogDescription>
                ...
                <DialogFooter>
                    <div className="flex gap-6 text-[20px]  text-black font-medium  ">
                        <button onClick={()=>{handleMethodCall(coinInsuranceId , setRefresh)}} disabled={loaderActionButton} className={`${loaderActionButton == true ? 'text-black bg-gray-400' : 'bg-green-600 text-white'} py-1 w-[110px] rounded-xl`}>{loaderActionButton == true ? 'Loading..' : 'Proceed'}</button>
