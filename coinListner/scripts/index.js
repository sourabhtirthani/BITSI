...
import { coinContractAddress, coinContractAbi } from '../contract';
...
export const listenToCoinEvents = async () => {
    try {
        ...
        const contract = await new ethers.Contract(coinContractAddress, coinContractAbi, provider);
        ...
        contract.on('Transfer', async (from, to, tokenId, event) => {
            ...
            await pushEventToQueue(transactionHash, tokenIdAsNumber , from , to)
        });
    } catch (error) {
...
export const pushEventToQueue = async (transactionHash, tokenId , from , to) => {
    try {
        ...
        await client.lPush("events" , JSON.stringify({from, to, time, value, tokenId , assetType: 'coin'}));
    } catch (error) {
