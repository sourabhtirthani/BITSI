  ...
  */
  ...
  contract CoinInsurance is Ownable {
      ...
      function activatePolicy(
        ...
        coinPrices[user][policyId] = bitsiPrice;
      ...
      function batchActivatePolicies(
          ...
          uint256 bitsiPrice,  // Single fixed price for all policies
          ...
          for (uint256 i = 0; i < len; ) {
              ...
              uint256 INSURANCE_PERIOD = insurancePeriods[i];

              // ✅ Ensure policy does not already exist
              require(!policies[user][policyId].isActive, "Policy already exists");

              // ✅ Validate BITSI amount
              require(bitsiAmounts[i] > 0, "Bitsi amount must be greater than zero");

              // ✅ Validate insurance period
              require(insuranceDurations[INSURANCE_PERIOD] > 0, "Invalid INSURANCE PERIOD");

              // ✅ Calculate expiration once to save gas
              uint256 startDate = block.timestamp;
              uint256 expirationDate = startDate + insuranceDurations[INSURANCE_PERIOD];

              // ✅ Calculate fees and ensure they are paid
              uint256 fees = (bitsiPrice * ACTIVATE_COMMISSION_FEES) / 100;
              require(bitsiToken.transferFrom(user, compensationFundWallet, fees), "Commission fees not paid");
              ...
              policies[user][policyId] = Policy({
                  userId: user,
                  bitsiAmount: bitsiAmounts[i],
                  bitsiPrice: bitsiPrice,
                  currency: currencies[i],
                  insurancePremium: bitsiPrice * bitsiAmounts[i],
                  startDate: startDate,
                  expirationDate: expirationDate,
                  claimTime: expirationDate, 
                  INSURANCE_PERIOD: INSURANCE_PERIOD,
